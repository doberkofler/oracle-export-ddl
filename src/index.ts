...
/**
 * @file CLI to export Oracle procedural objects from USER_SOURCE to physical files.
 */
...
const OBJECT_TYPE_EXT: Record<string, string> = {
	PACKAGE: '.pks',
	'PACKAGE BODY': '.pkb',
	PROCEDURE: '.prc',
	FUNCTION: '.fnc',
 ...
 * @param objectType
 */
const resolveExt = (objectType: string): string => OBJECT_TYPE_EXT[objectType.toUpperCase()] ?? '.pls';
 ...
 * @param filterName
 */
const exportObjects = async (connection: oracledb.Connection, directory: string, filterType: string | null, filterName: string | null): Promise<void> => {
